...
class TestMinioAdapter(BaseIntegrationTest):
    ...
    def test_should_delete_bucket_and_return_successfully(
        ...
        _minio_adapter = init_bootstrap.minio_adapter
        _random_name = f"{IdentityGenerator().get_random_id()}"
        _create_bucket = _minio_adapter.create_bucket(bucket_name=_random_name)
        assert _create_bucket is None
        # Act
        sut = _minio_adapter.delete_bucket(bucket_name=_random_name)
        # Assert
        assert sut is None
    ...
    def test_should_create_bucket_and_return_successfully(
        ...
        _random_name = f"{IdentityGenerator().get_random_id()}"
        # Act
        sut = _minio_adapter.create_bucket(bucket_name=_random_name)
        # Assert
        assert sut is None
        # Cleanup
        _delete_bucket = _minio_adapter.delete_bucket(bucket_name=_random_name)
    ...
    def test_should_create_existing_bucket_and_raise_s3error(
        ...
        # Act
        with pytest.raises(S3Error) as exec_info:
            _minio_adapter.create_bucket(bucket_name=_random_name)
            assert exec_info.type is S3Error
        # Cleanup
        _delete_bucket = _minio_adapter.delete_bucket(bucket_name=_random_name)
        assert _delete_bucket is None
    ...
    def test_should_check_bucket_exists_return_false(
        ...
        _random_name = f"{IdentityGenerator().get_random_id()}"
        # Act
        sut = _minio_adapter.bucket_exists(bucket_name=_random_name)
        # Assert
        assert sut is False
    ...
    def test_should_get_bucket_policy_and_return_successfully(
        ...
        # Act
        with pytest.raises(S3Error) as exec_info:
            _ = _minio_adapter.get_bucket_policy(bucket_name=_random_name)
            assert exec_info.type is S3Error
        # Cleanup
        _delete_bucket = _minio_adapter.delete_bucket(bucket_name=_random_name)
        assert _delete_bucket is None
    ...
    def test_should_get_bucket_lifecycle_and_return_successfully(
        ...
        assert _create_bucket is None
        # Act
        sut = _minio_adapter.get_bucket_lifecycle(bucket_name=_random_name)
        # Assert
        assert sut is None
        # Cleanup
        _delete_bucket = _minio_adapter.delete_bucket(bucket_name=_random_name)
    ...
    def test_should_list_buckets_and_return_empty_successfully(
        ...
        _minio_adapter = init_bootstrap.minio_adapter
        # Act
        sut = _minio_adapter.list_buckets()
        # Assert
    ...
    def test_should_list_buckets_and_return_few_successfully(
        ...
        for _each in _bucket_ids:
            _delete_bucket = _minio_adapter.delete_bucket(bucket_name=_each)
            assert _delete_bucket is None
        # Act
        sut = _minio_adapter.list_buckets()
        # Assert
