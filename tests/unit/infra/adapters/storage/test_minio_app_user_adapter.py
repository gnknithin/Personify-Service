...
class TestMinIOAppUserAdapter(BaseUnitTest):
    def test_should_return_an_typeerror_if_no_init_arg_is_informed(self) -> None:
        with pytest.raises(expected_exception=TypeError) as exec_info:
            sut = MinIOAppUserAdapter(logger=logging.Logger)
            assert sut is None
    ...
    def test_should_return_an_instance_of_mionio_app_user_adapter(self) -> None:
        # Arrange
        _config = self._get_dummy_connection()
        # Act
        sut = MinIOAppUserAdapter(**_config)
        # Assert
        assert isinstance(sut, MinIOAppUserAdapter)
